...
pacman::p_load(
  ...
  semPlot, randomForest, Metrics,corrplot, rsample, qgraph
)
```
...
model_results <- list()
# 设置交叉验证的参数
control <- trainControl(method = "cv", number = 10) # 10折 “cv”交叉验证；每次用其中9份训练，1份验证，循环10次
# 循环遍历每个变量
...
# 对具有高预测力的条目进行筛选
## r方和rmse
selected <- performance_df[performance_df$Rsquared > 0.3 & performance_df$RMSE < 0.8, ]
print(selected$Target_Variable)

## r方
selected <- performance_df[performance_df$Rsquared > 0.4, ]
print(selected$Target_Variable)
```
...
# 1. 去冗余（UVA）
uva <- UVA(data = data_z, wto.cut = 0.25)
data_ega <- uva$reduced_data
# 查看保留下来的条目名
kept_items <- colnames(uva$reduced_data)
kept_items
# 查看剔除掉的条目名
removed_items <- setdiff(colnames(data_z), colnames(uva$reduced_data))
removed_items

# 2. 运行 EGA
ega <- EGA(data = data_ega, model = "glasso", algorithm = "walktrap",
           corrMethod = "spearman", gamma = 0.5, plot.EGA = TRUE)
summary(ega)

# 3. 稳定性检测（BootEGA）
boot <- bootEGA(data = data_ega, model = "glasso", algorithm = "walktrap",
                corrMethod = "spearman", n.boot = 500)
summary(boot)

# 查看维度稳定性
dimensionStability(boot)
itemStability(boot)
```
```{r}
library(EGAnet)
...
# 节点名称
V(graph)$name <- colnames(ega$network)

# 社区信息（来自 EGA 的 wc）
V(graph)$community <- as.factor(ega$wc)
...
ggraph(graph, layout = "fr") +
  ...
  geom_node_point(aes(color = community), size = 8) +
...
factor_loadings <- subset(standardized_solution, op == "=~")
