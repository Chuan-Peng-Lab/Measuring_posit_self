...
# 加载所需要的R包
pacman::p_load("tidyverse","bruceR","ggplot2","ggridges","psych","psychTools","DataExplorer","patchwork","cowplot","ggpubr","BayesFactor","careless","rticles"," xaringan","igraph","brms","here")
...
```
虚报。所有都是0，代值进去以后的结果，哪个条件的虚报

```{r 计算ALT2_moral的SDT}
ALT2_moral_SDT<-ALT2_all%>%
  filter(domain=="ability")%>%
  fun_sdt_ALT2(.)
```
...
(1+Reference)
(0 +Reference:Valence + ismatch:Reference:Valence)
```
...
Referenceself:Valencenegative              -1.19      0.07    -1.34    -1.06 1.00     2540     2985  # c_self_negative =1.19
Referencefriend:Valencenegative            -1.13      0.07    -1.26    -1.00 1.00     2474     3373
Referenceself:Valencepositive              -1.08      0.07    -1.22    -0.95 1.00     2668     3388
Referencefriend:Valencepositive            -1.07      0.07    -1.21    -0.93 1.00     2213     3257
Referenceself:Valencenegative:ismatch       2.46      0.13     2.21     2.73 1.00     2321     2643  # d_prime_self_negative=2.46
Referencefriend:Valencenegative:ismatch     2.13      0.13     1.87     2.39 1.00     2544     3130
Referenceself:Valencepositive:ismatch       2.77      0.20     2.37     3.17 1.00     2945     3242
Referencefriend:Valencepositive:ismatch     2.35      0.15     2.08     2.65 1.00     2313     2664


c_self_positive-c_friend_positive
c_self_negative-c_friend_negative

回归系数的意义

d_prime_self_positive-d_prime_friend_positive
d_prime_self_negative-d_prime_friend_negative

```{r}
fun_plot_sdt_val <- function(m_sdt) {
        # extract c
        tmp_c <- m_sdt %>% 
                tidybayes::gather_draws(b_Valencenegative, b_Valencepositive) %>%
                dplyr::rename(Valence = .variable, sdt_c = .value) %>% dplyr::ungroup() %>%
                dplyr::mutate(Valence = gsub("b_", "", Valence)) %>%
                dplyr::mutate(Valence = ifelse(stringr::str_detect(Valence, 'negative'), 'negative',
                                               ifelse(stringr::str_detect(Valence, 'positive'), 'positive')))
        
        # dprime
        tmp_d <- m_sdt %>%                                                                                                                  
                tidybayes::gather_draws(`b_Valencenegative:ismatch`,  
                                        `b_Valencepositive:ismatch`) %>%
        ...
        p_sdt_d_sum <- tmp_d %>%
                dplyr::mutate(Valence = factor(Valence, levels = c('negative','positive'))) %>%
...
df_m1_std_fixed_effect <- bayestestR::describe_posterior(
        ...
        dplyr::mutate(Valence = dplyr::case_when(
                grepl("Valencenegative", Parameter) ~ "negative",
                grepl("Valencepositive", Parameter) ~ "positive"),
...
###################################################################
df_m1_std_fixed_effect2 <- bayestestR::describe_posterior(
        ALT2_moral_SDT,
        effects = "fixed",
        component = "all",
        ci = 0.95,
        ...
        centrality = "all") %>%
        dplyr::mutate(Valence = dplyr::case_when(
                grepl("Referenceself", Parameter) ~ "self",
                grepl("Referencefriend", Parameter) ~ "friend"),
                params = dplyr::case_when(grepl("ismatch", Parameter) ~ "d prime",
                                           !grepl("ismatch", Parameter) ~"criterion"),
                params = factor(params, levels = c('d prime', 'criterion')),
                Valence = factor(Valence, levels = c('self','friend'))) 
#####################################################################
```
...
df_m1_plot_sdt <- df_sdt_m1_pop %>%
        ...
        dplyr::mutate(condition = factor(Reference, levels = c("self", "friend")),
                      condition = forcats::fct_rev(Rference), # reverse the order because the plot function auto reverse.
...
fun_plot_sdt_val (ALT2_moral_SDT)
```
...
check_SRET<-function(data){
  ...
  filter(screen_id%in%c("EW_formal") )%>% #选择正式实验的数据
   filter(!word %in% c("务实", "迷糊", "坚贞", "说谎", "主见", "缓慢", "素养", "低俗")) %>%
   group_by(ID) %>%
  summarize(EW_rt = sum(rt, na.rm = TRUE),
            EW_rt=EW_rt/60000)
print(SRET_EW_Time)
 ...
 SRET_RJ_Time <- data %>%
    ...
    mutate(sdt = case_when((identity == "old" & (responses %in% c("familiar", "old"))) ~ "hit",
                          ...
                          (identity == "new" & (responses == "new")) ~ "cr"),) %>%
   group_by(ID) %>%
  summarize(RJ_rt = sum(rt, na.rm = TRUE),
            RJ_rt=RJ_rt/60000)
print(SRET_RJ_Time)
...
SRET_RJ_2_Time <- data %>%
 ...
 mutate(correct = as.numeric(coalesce(correct,-1))) %>% #对新词按键判断的试次的correct记为-1
  ...
  filter(!word %in% c("严谨", "认真", "刻板", "白痴", "忠实", "宽宏", "徇私", "可鄙")) %>%
   ...
   group_by(ID) %>%
  summarize(RJ2_rt = sum(rt, na.rm = TRUE),
            RJ2_rt=RJ2_rt/60000)
print(SRET_RJ_2_Time)
SRET_Time <- left_join(SRET_EW_Time, SRET_RJ_Time, by = "ID")%>%
  left_join(., SRET_RJ_2_Time, by = "ID")
print(SRET_Time)
assign("SRET_Time", SRET_Time, envir = .GlobalEnv)
SRET_Time_avg<-SRET_Time%>%
  select(-ID)%>%
  summarise(avg_EW_rt = mean(EW_rt, na.rm = TRUE),
    avg_RJ_rt = mean(RJ_rt, na.rm = TRUE),
    avg_RJ2_rt = mean(RJ2_rt, na.rm = TRUE))
    print(SRET_Time_avg)
...
# 将三个被试 ID 合并为一个数据框
all_IDs_df <- data.frame(ID = unique(c(SRET_EW_subj$ID, SRET_RJ_subj$ID, SRET_RJ_2_subj$ID)))
SRET_Time<-SRET_Time%>%
   left_join(., SRET_math, by = "ID")%>%
  filter(ID %in% all_IDs_df$ID)
# 输出结果
print(SRET_Time)
```
```{r}
SRET_RJ_avg<-SRET_RJ%>%
   filter(!ID %in% all_IDs_df$ID)%>%
  select(-ID)%>%
  group_by(domain,valence)%>%
  summary(avg_recognition=mean(recognition),
          avg_Correct_recognition=mean(Correct_recognition),
