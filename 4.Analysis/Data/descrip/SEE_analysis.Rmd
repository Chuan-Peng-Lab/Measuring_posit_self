...
for (subfolder in subfolders) {
  phase_folder <- file.path(subfolder, "phase_004")# 循环读取每个子文件夹下的 phase_0xx 文件夹内的 CSV 文件
...
subj_phase_004 <- unique(day3_q_phase_004$ID)
print(subj_phase_004)
```
...
3.within MANOVA(within.3, dvs="A1B1C1:A2B2C2", dvs.pattern="A(.)B(.)C(.)",
       within=c("A", "B", "C"))
```{r 按匹配与不匹配绘制acc和RT}
ALT1_plot_phase_004 <- subset(ALT1_phase_004, ID %in% subj_phase_004)%>%
...
```
```{r}
ALT1_p2_phase_004 <- subset(ALT1_phase_004, ID %in% subj_phase_004)%>%
 # filter(., ID != "phase_003_subj_14")%>%#不需要每次有这行代码，本次去除是因为被试重复作答
  mutate(ID = as.character(ID)) %>%
   mutate(ParticipantID = as.character(ParticipantID)) %>%
  mutate(rt=as.numeric(rt))%>% #转变被试编号和反应时类型为字符型与数值型
  mutate(correct = ifelse(correct == "true", 1, ifelse(correct == "false", 0, NA)))%>% ##重编码correct，1对0错
  group_by(screen_id) %>%  #按被试与条件分组
  summarise(
    avg_rt = mean(rt, na.rm = TRUE),
    max_rt = max(rt, na.rm = TRUE),
    min_rt = min(rt, na.rm = TRUE),
...
ALT1_plot <- acc_and_rt_plots(ALT1_plot_phase_004)
...
# 使用函数并传递ALT1_plot_phase_003作为参数
ALT1_plot2 <- acc_and_rt_plots2(ALT1_plot2_phase_004)
...
ALT2_p_phase_004 <- subset(ALT2_phase_004, ID %in% subj_phase_004)%>%
 # filter(., ID != "phase_003_subj_14")%>%#不需要每次有这行代码，本次去除是因为被试重复作答
  mutate(ID = as.character(ID)) %>%
   mutate(ParticipantID = as.character(ParticipantID)) %>%
  mutate(rt=as.numeric(rt))%>% #转变被试编号和反应时类型为字符型与数值型
  group_by(screen_id,identity,person,valence) %>%  #按被试与条件分组
  summarise(
    avg_rt = mean(rt, na.rm = TRUE),
    max_rt = max(rt, na.rm = TRUE),
    min_rt = min(rt, na.rm = TRUE),
...
ALT2_acc_plot(ALT2_plot_phase_004)
```
```{r ALT2_valence}
...
ALT2_acc_plot2(ALT2_plot_phase_004)
```
